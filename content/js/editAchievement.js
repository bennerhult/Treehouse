treehouseApp.controller('editAchievementController', function($scope,  $http, achievementService, pageService) {
    pageService.setTitle('Edit achievement');
    
    $scope.achievement = angular.copy(achievementService.achievement);
    ...
    $scope.isValidState = function () {
        return $scope.achievement.title && $scope.achievement.description
    }

    $scope.editAchievement = function(evt) {
        var changes, changeCount;
        changes = {};
        changeCount = 0;
        if($scope.achievement.title != achievementService.achievement.title) {
            changes.title = $scope.achievement.title;
            achievementService.achievement.title = changes.title;
            changeCount = changeCount + 1;
        }
        if($scope.achievement.description != achievementService.achievement.description) {
            changes.description = $scope.achievement.description;
            achievementService.achievement.description = changes.description;
            changeCount = changeCount + 1;
        }
        if(changeCount > 0) {
            $http.post('/api/achievements/editAchievement', {
                achievementId : $scope.achievement.achievementId,
                changes : changes
            })
        }
    }
